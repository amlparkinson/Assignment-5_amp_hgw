...
**I dont tihnk the rest of this is necessary** its confusing when the counts are referenced bc the point of looking at proportions is bc actual counts can be misleading 

This can be seen in Table 1 - the counts in clear cut and old-growth channel types were not different enough for the chi-squared test to find a significant difference. For example, Salamander counts were actually higher in clear cut sections in the forest for side channels (n~side channel~ = 90) and cascade (n~cascade~ = 247) channel types. Pools were the only channel type to gave higher salamander counts in old-growth forests (n~pool~ = 45). 
...
A two-sample t-test was used to determine if mean salamander weights in 2017 were significantly different in old-growth and clear cut forests. 2017 mean salamander weights in clear cut forests (`r round(weigths_ttest$estimate[1],2)` grams) did not differ significantly from 2017 salamander weights in old-growth forests (`r round(weigths_ttest$estimate[2],2)` grams) (t(`r round(weigths_ttest$parameter,2)`) = `r round(weigths_ttest$statistic,2)`, *p* = `r round(weigths_ttest$p.value,2)`). A negligable effect size (`r round(weights_effectsize$estimate,1)`) confirms that there is no difference in mean 2017 weights by section along Mack Creek.


#### Results E: Is there a significant difference between mean salamander weights in the 3 channel types of Mack Creek in 2017?

Salamanders found in 2017 in side channels of Mack Creek had the lowest mean weight (mean~2017~ = 5.68 grams) compared to salamanders found in pools (mean~2017~ = 9.30 grams) and cascades (mean~2017~ = 7.52 grams) (Figure 4.).
...
# Limit y-axis to zero so sd bars aren't negative.
```
...
**amp can finish up later bc its late, talk about the results of the levene test and anovas being robust to normality assumption and non parametric tests have less statistical power**
...
- Mean salamander weights were only significantly different in side channels and pools (Figure 4).
